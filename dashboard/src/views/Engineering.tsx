...
type GroupKey = 'projectConfig' | 'project' | 'config' | 'transmission' | 'cycle'
...
function Deterioration({ rows }: { rows: ReturnType<typeof applyFilters> }) {
  ...
  const [groupBy, setGroupBy] = useState<GroupKey>('projectConfig')
  const [ul, setUl] = useState(160000)

  const groups = useMemo(
    () => deteriorationByGroup(rows, poll, (t) => groupValue(t, groupBy), ul, {
      targetFor: targetProfile,
      normFor: (t) => (isRegulatoryBasisConfirmed(t) ? regulatoryProfile(t) : null),
    }),
    [rows, poll, groupBy, ul],
  )
  const targetLimit = commonGroupLimit(groups.map((g) => g.target), groups.some((g) => g.mixedTarget))
  const normLimit = commonGroupLimit(groups.map((g) => g.norm), groups.some((g) => g.mixedNorm))
  const target = targetLimit.value
  const norm = normLimit.value
  const mixedTarget = targetLimit.mixed
  const mixedNorm = normLimit.mixed
      ...
      <Panel ticks={false}>
        <div className="panel-heading">
          ...
          <span className="legend-inline">
            <i className="target-line" /> target {target != null ? fmt(target, poll, massUnit) : mixedTarget ? 'mixed by group' : '—'}
            {' · '}norm {norm != null ? fmt(norm, poll, massUnit) : mixedNorm ? 'mixed by group' : '—'}
          </span>
        </div>
            ...
            <tbody>
              ...
              {groups.map((g) => (
                <tr key={g.group} style={g.reliable ? undefined : { opacity: 0.72 }}>
                  ...
                  <td>{!g.reliable ? <Verdict tone="na">Insufficient spread</Verdict> : g.mixedNorm || g.mixedTarget ? <Verdict tone="na">Mixed limits</Verdict> : g.exceedsNorm ? <Verdict tone="fail">Breaches norm aged</Verdict> : g.exceedsTarget ? <Verdict tone="warn">Breaches target aged</Verdict> : <Verdict tone="pass">Holds to life</Verdict>}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div className="analysis-note">Linear least-squares of {poll} vs odometer; projected to {ul.toLocaleString()} km. DF = projected ÷ current. Targets are applied only when the group has one unambiguous project target; regulatory lines use confirmed basis only.</div>
      </Panel>
    </>
  )
}
...
function Conformity({ rows }: { rows: ReturnType<typeof applyFilters> }) {
  ...
  const stats = useMemo(() => LIMITED.map((p) => {
    ...
    const targetVals = rows.map((t) => ({ t, c: targetCompliance(t) })).filter((x) => x.c?.perPollutant[p].margin != null && x.t.results[p] != null)
    const normVals = rows.map((t) => ({ t, c: confirmedRegulatoryCompliance(t) })).filter((x) => x.c?.perPollutant[p].margin != null && x.t.results[p] != null)
    const target = commonComplianceLimit(targetVals.map(limitFromCompliance(p)))
    const norm = commonComplianceLimit(normVals.map(limitFromCompliance(p)))
    const tgtPass = targetVals.filter((x) => (x.c!.perPollutant[p].margin ?? -1) >= 0).length
    return {
      p, n: vals.length, mean: mean(vals), sd: stdev(vals),
      cpkTarget: target.value == null ? null : cpu(targetVals.map((x) => x.t.results[p]!).filter((v): v is number => v != null), target.value),
      cpkNorm: norm.value == null ? null : cpu(normVals.map((x) => x.t.results[p]!).filter((v): v is number => v != null), norm.value),
      tgtPass,
      normPass: normVals.filter((x) => (x.c!.perPollutant[p].margin ?? -1) >= 0).length,
      target: target.value,
      norm: norm.value,
      mixedTarget: target.mixed,
      mixedNorm: norm.mixed,
    }
  }).filter((s): s is NonNullable<typeof s> => !!s && s.n > 0), [rows])

  const targetRows = rows.map((t) => targetCompliance(t)).filter(Boolean)
  const overallTargetPass = targetRows.filter((c) => c!.overall !== 'fail').length
  const normRows = rows.map((t) => confirmedRegulatoryCompliance(t)).filter(Boolean)
  const overallNormPass = normRows.filter((c) => c!.overall !== 'fail').length
  const wTarget = wilson(overallTargetPass, targetRows.length)
  const wNorm = wilson(overallNormPass, normRows.length)
    ...
    <>
      <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(2, minmax(0,1fr))' }}>
        <Panel><div className="metric-card">
          ...
          <span>{overallTargetPass}/{targetRows.length} · true rate {(wTarget.lo * 100).toFixed(0)}–{(wTarget.hi * 100).toFixed(0)}% (Wilson)</span>
        </div></Panel>
        <Panel><div className="metric-card">
          <Eyebrow>Fleet pass vs resolved regulation · 95% CI</Eyebrow>
          <div className="metric-value" style={{ color: wNorm.p === 1 ? RAG_COLOR.pass : RAG_COLOR.fail }}>{(wNorm.p * 100).toFixed(0)}%</div>
          <span>{overallNormPass}/{normRows.length} confirmed · true rate {(wNorm.lo * 100).toFixed(0)}–{(wNorm.hi * 100).toFixed(0)}% (Wilson)</span>
        </div></Panel>
      </div>
            ...
            <tbody>
              {stats.map((s) => (
                <tr key={s.p}>
                  ...
                  <td className="font-mono">{s.mixedTarget ? 'mixed' : fmt(s.target, s.p, massUnit)}</td>
                  ...
                  <td className="font-mono">{s.tgtPass}/{rowsWithPollutantTarget(rows, s.p)}</td>
                  <td className="font-mono">{s.normPass}/{rowsWithPollutantNorm(rows, s.p)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <>
      <Panel ticks={false}>
        ...
        <div className="analysis-note">
          Accept/reject per AIS-175 Annex B6 §2.6.8.3.1.3: IWR must fall strictly within <strong>−2.0 … +4.0 %</strong>{' '}
          — the band is asymmetric, and a run outside it is not a valid Type-I trace and should be
          re-driven. RMSSE below 1.3 km/h (aim below 1.0). Driven distance within ±1 % of the schedule;
...
const groupHeader = (k: GroupKey) => (
  k === 'projectConfig' ? 'Program / configuration'
    : k === 'project' ? 'Program'
...
const groupValue = (t: ReturnType<typeof applyFilters>[number], k: GroupKey): string => {
...
function commonGroupLimit(values: (number | null)[], anyMixed = false): { value: number | null; mixed: boolean } {
  if (anyMixed) return { value: null, mixed: true }
...
function commonComplianceLimit(values: (number | null)[]): { value: number | null; mixed: boolean } {
...
const limitFromCompliance = (p: Pollutant) => (row: { t: ReturnType<typeof applyFilters>[number]; c: NonNullable<ReturnType<typeof targetCompliance>> | NonNullable<ReturnType<typeof confirmedRegulatoryCompliance>> | null }): number | null => {
  const value = row.t.results[p]
  const margin = row.c?.perPollutant[p].margin
  return value == null || margin == null || margin >= 1 ? null : value / (1 - margin)
}

function rowsWithPollutantTarget(rows: ReturnType<typeof applyFilters>, p: Pollutant): number {
  return rows.filter((t) => t.results[p] != null && targetCompliance(t)?.perPollutant[p].margin != null).length
}

function rowsWithPollutantNorm(rows: ReturnType<typeof applyFilters>, p: Pollutant): number {
  return rows.filter((t) => t.results[p] != null && confirmedRegulatoryCompliance(t)?.perPollutant[p].margin != null).length
}
